...
//import {TokenDistributorOriginal} from "../src/TokenDistributorOriginal.sol";
import {TokenDistributorOriginal} from "../src/TokenDistributorOriginal.sol";
import {FeeSharingSystem} from "../src/FeeSharingSystem.sol";
import {FeeSharingSetter} from "../src/FeeSharingSetter.sol";

import {AggregatorFeeSharingWithUniswapV3} from "../src/AggregatorFeeSharingWithUniswapV3.sol";
...
contract AggregatorTest is TestParameters, TestHelpers {
    ...
    TokenDistributorOriginal public tokenDistributor;
    ...
    function setUp() public {
        ...
        // 3. TokenDistributorOriginal deployment
        ...
        // 4. TokenDistributorOriginal deployment
        tokenDistributor = new TokenDistributorOriginal(
        ...
        feeSharingSetter = new FeeSharingSetter(
            address(feeSharingSystem),
            30,
            1000,
            100
        );
        feeSharingSetter.grantRole(
            feeSharingSetter.OPERATOR_ROLE(),
            feeSharingSystem.owner()
        );
    ...
    function testDeposit() public asPrankedUser(user1) {
        ...
        assertEq(
            aggregatorFeeSharingWithUniswapV3.calculateSharePriceInLOOKS(),
            _parseEtherWithFloating(13, 1)
        );
        ...
        assertEq(
            ...
            _parseEther(130) + currentBalanceUser1
        );
        assertEq(
            aggregatorFeeSharingWithUniswapV3.calculateSharesValueInLOOKS(
                user1
        ...
        assertEq(
            ...
            _parseEther(200 + 49 * 30)
        );
    }

    function testSameBlockDepositAndWithdraw(
        uint8 x,
        uint16 numberBlocks
        ...
        cheats.assume(
            amountDeposit >=
                aggregatorFeeSharingWithUniswapV3.MINIMUM_DEPOSIT_LOOKS()
        );
        ...
        assertEq(
            looksRareToken.balanceOf(user1),
            currentBalanceUser1 + amountDeposit
        );
    }

    function testSameBlockMultipleDeposits(
        uint8 x,
        uint16 numberBlocks
    ) public {
        uint256 amountDeposit = _parseEther(x);
        ...
        assertEq(
            aggregatorFeeSharingWithUniswapV3.userInfo(user1),
            aggregatorFeeSharingWithUniswapV3.userInfo(user2)
        );
    }
    ...
    function testScenarioWithoutRouter() public {
        ...
        assertEq(
            looksRareToken.balanceOf(user1),
            currentBalanceUser +
                _parseEtherWithFloating(1125, 1) +
                amountDeposit
        );
        ...
        assertQuasiEq(
            ...
            currentBalanceUser + _parseEtherWithFloating(13625, 1)
        );
        ...
        // Total = 2150 LOOKS
        assertQuasiEq(
            looksRareToken.balanceOf(user3),
            currentBalanceUser + _parseEther(2150)
        );
        ...
        assertQuasiEq(
            looksRareToken.balanceOf(user4),
            currentBalanceUser + _parseEther(2150)
        );

        // Verify the final total supply is equal to the cap - supply not minted (for first 5 blocks)
        assertEq(
            looksRareToken.totalSupply(),
            _parseEther(_CAP) - _parseEther(500)
        );
    }
    ...
    function testScenarioWithRouter() public {
        ...
        aggregatorFeeSharingWithUniswapV3.updateMaxPriceOfLOOKSInWETH(
            _parseEtherWithFloating(667, 3)
        ); // 1 LOOKS = 0.667 WETH
        ...
        assertEq(
            looksRareToken.balanceOf(user1),
            currentBalanceUser +
                _parseEtherWithFloating(16875, 2) +
                amountDeposit
        );
        ...
        assertEq(
            aggregatorFeeSharingWithUniswapV3.calculateSharePriceInLOOKS(),
            _parseEtherWithFloating(26875, 4)
        );
        ...
        assertEq(
            ...
            _parseEtherWithFloating(2125, 3)
        );
        ...
        assertEq(
            aggregatorFeeSharingWithUniswapV3.calculateSharePriceInPrimeShare(),
            (aggregatorFeeSharingWithUniswapV3.calculateSharePriceInLOOKS() *
                1e18) / feeSharingSystem.calculateSharePriceInLOOKS()
        );

        // User1 decides to re-deposit the exact same amount as the one earned
        cheats.prank(user1);
        aggregatorFeeSharingWithUniswapV3.deposit(
            _parseEtherWithFloating(16875, 2) + amountDeposit
        );
        ...
        assertQuasiEq(
            looksRareToken.balanceOf(user1),
            currentBalanceUser +
                _parseEtherWithFloating(106875, 2) +
                amountDeposit
        );
        ...
        assertQuasiEq(
            rewardToken.balanceOf(address(feeSharingSystem)),
            _parseEther(50)
        );
