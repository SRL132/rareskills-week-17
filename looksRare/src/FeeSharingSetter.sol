...
contract FeeSharingSetter is ReentrancyGuard, AccessControl {
    ...
    function updateRewards() external onlyRole(OPERATOR_ROLE) {
        ...
        if (numberAddressesForFeeStaking > 0) {
            ...
            (uint256 totalAmountStaked, ) = tokenDistributor.s_userInfo(
     ...
     * @param _stakingAddresses array of addresses eligible for fee-sharing only
     */
    function removeFeeStakingAddresses(
        address[] calldata _stakingAddresses
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _stakingAddresses.length; i++) {
            require(
     ...
     */
    function setNewRewardDurationInBlocks(
        uint256 _newRewardDurationInBlocks
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
     ...
     */
    function setRewardConvertor(
        address _rewardConvertor
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function transferOwnershipOfFeeSharingSystem(
        address _newOwner
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _newOwner != address(0),
