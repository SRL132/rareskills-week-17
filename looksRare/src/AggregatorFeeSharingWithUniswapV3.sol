 ...
 */
contract AggregatorFeeSharingWithUniswapV3 is
    Ownable,
    Pausable,
    ReentrancyGuard
{
    ...
    constructor(
        ...
        address looksRareTokenAddress = address(
            FeeSharingSystem(_feeSharingSystem).looksRareToken()
        );
        address rewardTokenAddress = address(
            FeeSharingSystem(_feeSharingSystem).rewardToken()
        );
        ...
        IERC20(looksRareTokenAddress).approve(
            _feeSharingSystem,
            type(uint256).max
        );
        ...
        MINIMUM_DEPOSIT_LOOKS = FeeSharingSystem(_feeSharingSystem)
            .PRECISION_FACTOR();
    }
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 totalAmountStaked = feeSharingSystem
            .calculateSharesValueInLOOKS(address(this));
     ...
     * @dev Only callable by owner
     */
    function updateMaxPriceOfLOOKSInWETH(
        uint256 _newMaxPriceLOOKSInWETH
    ) external onlyOwner {
     ...
     */
    function updateTradingFeeUniswapV3(
        uint24 _newTradingFeeUniswapV3
    ) external onlyOwner {
        require(
            _newTradingFeeUniswapV3 == 10000 ||
                _newTradingFeeUniswapV3 == 3000 ||
    ...
    function calculateSharePriceInLOOKS() external view returns (uint256) {
        uint256 totalAmountStakedWithAggregator = feeSharingSystem
            .calculateSharesValueInLOOKS(address(this));

        return
            totalShares == 0
                ? MINIMUM_DEPOSIT_LOOKS
                : (totalAmountStakedWithAggregator * MINIMUM_DEPOSIT_LOOKS) /
                    (totalShares);
    }
    ...
    function calculateSharePriceInPrimeShare() external view returns (uint256) {
        (uint256 totalNumberPrimeShares, , ) = feeSharingSystem.userInfo(
            address(this)
        );
        ...
        return
            totalShares == 0
                ? MINIMUM_DEPOSIT_LOOKS
                : (totalNumberPrimeShares * MINIMUM_DEPOSIT_LOOKS) /
                    totalShares;
    }
     ...
     */
    function calculateSharesValueInLOOKS(
        address user
    ...
    function _harvestAndSellAndCompound() internal {
        ...
        if (amountToSell >= thresholdAmount) {
            ...
            if (isExecuted) {
                uint256 adjustedAmount = looksRareToken.balanceOf(
                    address(this)
                );
    ...
    function _sellRewardTokenToLOOKS(uint256 _amount) internal returns (bool) {
        uint256 amountOutMinimum = maxPriceLOOKSInWETH != 0
            ? (_amount * 1e18) / maxPriceLOOKSInWETH
            : 0;
    ...
    function _withdraw(uint256 _shares) internal {
        ...
        uint256 amountToTransfer = looksRareToken.balanceOf(address(this)) -
            previousBalanceLOOKS;
